package com.fungames.galaxyshooter.plane;
        ...
        if (isAlive) {
            if (!isExplosion) {
                if (isVisible) {
                    ...
                    canvas.drawBitmap(reloadPlaneBitMap(smallPlane, R.drawable.small), object_x, object_y, paint);
                    canvas.restore();
                }
                logic();
            } else {
                ...
                canvas.drawBitmap(reloadPlaneBitMap(smallPlane, R.drawable.small), object_x, object_y - y, paint);
                canvas.restore();
                currentFrame++;
                if (currentFrame >= 3) {
                    currentFrame = 0;
